...
const LeagalInformation = () => {
  ...
  const handleSaveLegalInfo = async () => {
    ...
    try{
      ...
      const res = await api.put("/restaurant/update-legal-info",legalInfoFormData)
      ...
      toast.error(
        error.response?.data?.message || "Unknown error occurred updating restaurant. Please try again"
      )
    } finally{
      setIsLoading(false);
    }
